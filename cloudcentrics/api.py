...
def cloudcentrics_patch():
    ...
    update_field_label()
    if cint(get_frappe_version()) >= 13 and not frappe.db.get_single_value('Cloudcentrics Setting', 'ignore_onboard_cloudcentrics'):
...
def boot_session(bootinfo):
    ...
    if frappe.session['user'] != 'Guest':
        ...
        bootinfo.cloudcentrics_setting = frappe.get_doc(
            "Cloudcentrics Setting", "Cloudcentrics Setting")


@frappe.whitelist()
def ignore_update_popup():
    if not frappe.db.get_single_value('Cloudcentrics Setting', 'disable_new_update_popup'):
...
def show_update_popup_update():
    ...
    if update_message:
        frappe.msgprint(update_message, title=_(
            "New updates are available"), indicator='green')
